...
public class ClassicRoundelDoorBlock extends HorizontalFacingBlock implements BlockEntityProvider {

    public static final BooleanProperty OPEN = BooleanProperty.of("open");
    public static final DirectionProperty FACING = Properties.HORIZONTAL_FACING;

    public ClassicRoundelDoorBlock(Settings settings) {
        super(settings);
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return AITExtrasBlockEntityTypes.CLASSIC_ROUNDEL_DOOR_BLOCK_ENTITY_TYPE.instantiate(pos, state);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(@NotNull World world, @NotNull BlockState state,
        ...
        return (world1, blockPos, blockState, ticker) -> {
            if (ticker instanceof ClassicRoundelDoorBlockEntity be) {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos,
        ...
        if (world.getBlockEntity(pos) instanceof ClassicRoundelDoorBlockEntity be) {

            if (!world.isClient) {
                boolean isOpen = state.get(ClassicRoundelDoorBlock.OPEN);
                world.setBlockState(pos, state.with(ClassicRoundelDoorBlock.OPEN, !isOpen), Block.NOTIFY_ALL);
